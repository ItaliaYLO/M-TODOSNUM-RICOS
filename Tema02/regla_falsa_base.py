def regla_falsa(f, a, b, tol=1e-5, max_iter=100):
    """
    Implementación genérica del Método de Regla Falsa (Regula Falsi).
    f: Función a evaluar
    a, b: Límites del intervalo inicial
    tol: Tolerancia del error relativo aproximado
    """
    if f(a) * f(b) >= 0:
    ...
    for i in range(1, max_iter + 1):
        ...
        # Validación del cambio de signo para actualizar los intervalos
        if f_a * f_xr < 0:
            b = xr
        else:
            a = xr
